...
df <- read.csv(file = "../resources/be/vlaanderen/omgeving/data/id/dataset/codelijst-emissie/catalog.csv", sep=",", na.strings=c("","NA"))
df <- df %>%
...
write.csv(df,"../resources/be/vlaanderen/omgeving/data/id/dataset/codelijst-emissie/catalog.csv", row.names = FALSE)
df <- df %>%
  mutate_all(list(~ str_c("", .)))
...
context <- jsonlite::read_json("../resources/be/vlaanderen/omgeving/data/id/dataset/codelijst-emissie/context.json")
df_in_list <- list('@graph' = df, '@context' = context)
df_in_json <- toJSON(df_in_list, auto_unbox=TRUE)
write(df_in_json, "/tmp/catalog.jsonld")
system("riot --formatted=TURTLE /tmp/catalog.jsonld > ../resources/be/vlaanderen/omgeving/data/id/dataset/codelijst-emissie/catalog.ttl")
system("riot --formatted=JSONLD ../resources/be/vlaanderen/omgeving/data/id/dataset/codelijst-emissie/catalog.ttl > ../resources/be/vlaanderen/omgeving/data/id/dataset/codelijst-emissie/catalog.jsonld")
